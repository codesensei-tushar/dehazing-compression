...
#
# Uses CLUSTER_PYTHON env var if set, else /home/teaching/miniconda3/envs/adu/bin/python
...
if ! command -v tmux >/dev/null; then
    ...
    nohup bash -c "
        set -e
        cd '${PROJECT_DIR}'
        '${PY}' phase1_quantize/run_all_ptq.py --variants fp32 dyn_all dyn_mixed --top-k 5 --n-eval ${N_EVAL} \
            > results/phase1_runall.log 2>&1
        '${PY}' phase1_quantize/block_static_ptq.py --n-calib ${N_CALIB} --n-eval ${N_EVAL} --top-k 5 \
            > results/phase1_block_static.log 2>&1
        echo PHASE1_DONE \$(date -Iseconds) > results/phase1_status.txt
...
# Keep Torch thread pool modest so we're a good citizen on a shared box.
tmux send-keys -t "$SESSION" "export OMP_NUM_THREADS=16 MKL_NUM_THREADS=16" C-m
...
tmux send-keys -t "$SESSION" "${PY} phase1_quantize/run_all_ptq.py --variants fp32 dyn_all dyn_mixed --top-k 5 --n-eval ${N_EVAL} 2>&1 | tee results/phase1_runall.log" C-m
...
tmux send-keys -t "$SESSION" "${PY} phase1_quantize/block_static_ptq.py --n-calib ${N_CALIB} --n-eval ${N_EVAL} --top-k 5 2>&1 | tee results/phase1_block_static.log" C-m
